...
#                    print("Grad ", func.name, "Var: ", var.name, " ", grads[i][j])
    ...
    def verification_test_2(self):
        ...
        # Compute one step of the forward solution
        self.driver.solvers['flow'].initialize(scenario, bodies)
        self.driver.solvers['flow'].iterate(scenario, bodies, step)
        self.driver.solvers['flow'].post(scenario, bodies)
        ...
        self.driver.solvers['flow'].initialize_adjoint(scenario, bodies)
        self.driver.solvers['flow'].iterate_adjoint(scenario, bodies, step)
        self.driver.solvers['flow'].post_adjoint(scenario, bodies)
        ...
        adjoint_product = self.driver.solvers['flow'].comm.allreduce(adjoint_product)
        ...
        fd_product = self.driver.solvers['flow'].comm.allreduce(fd_product)

        if self.driver.solvers['flow']  .comm.rank == 0:
            print('FUN3D FUNtoFEM adjoint result:           ', adjoint_product)
            print('FUN3D FUNtoFEM finite-difference result: ', fd_product)
