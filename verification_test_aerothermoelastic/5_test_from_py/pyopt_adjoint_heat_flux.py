...
#                    print("Grad ", func.name, "Var: ", var.name, " ", grads[i][j])
    ...
    def verification_test(self):
        ...
        plate = self.model.bodies
        
        #solve_forward()
        self.driver._distribute_variables(steady, plate)
        self.driver._distribute_functions(steady, plate)
        self.driver._initialize_forward(steady, plate)
        self.driver._update_transfer()
        self.driver._solve_steady_forward(steady)
        self.driver._post_forward(steady, plate)
        #self.driver._eval_functions(steady, plate)

        HeatFlux1 = np.array(plate[0].aero_heat_flux[3::4])
        E = 1e-6
        deltaT = np.ones(np.array(plate[0].aero_temps[:]).shape)
        plate[0].aero_temps[:] = plate[0].aero_temps[:] + E*deltaT
        ...
        HeatFlux2 = np.array(plate[0].aero_heat_flux[3::4])
       
        #solve_adjoint()
        self.driver._distribute_variables(steady, plate)
        self.driver._distribute_functions(steady, plate)
        self.driver._initialize_adjoint_variables(steady, plate)
        self.driver._initialize_adjoint(steady, plate)
        
        lam_mag_thermal = np.ones(np.array(plate[0].dQdfta[3::4]).shape)
        plate[0].dQdfta[3::4] = lam_mag_thermal
        
        self.driver._solve_steady_adjoint(steady)
        self.driver._post_adjoint(steady, plate)
        #self.driver._eval_function_grads(steady)

        lam_t_temp = np.array(plate[0].dAdta[:,2])

        #Finite Difference
        lamH = lam_mag_thermal[:,2]
        output1 = (HeatFlux2 - HeatFlux1) / E
        output1 = np.dot(output1, lamH)
        output2 = np.dot(lam_t_temp,deltaT)
        Error = abs((output1-output2)/output1)
