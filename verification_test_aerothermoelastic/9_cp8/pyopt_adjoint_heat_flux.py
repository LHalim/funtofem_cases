...
#                    print("Grad ", func.name, "Var: ", var.name, " ", grads[i][j])
    ...
    def verification_test(self):
        ...
        body.dLdfa = np.random.uniform(low=1.0,high=1.0,size=body.dLdfa.shape)

        # ADJOINT SOLVE
        self.adjoint_solve(scenario,bodies)       

        adjoint_product = 0.0
        body.aero_disps_pert = np.random.uniform(low=1.0,high=1.0,size=body.aero_disps.shape)
