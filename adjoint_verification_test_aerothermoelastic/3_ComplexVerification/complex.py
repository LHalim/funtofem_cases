...
class wedge_adjoint(object):
    ...
    def verification_test(self):
        ...
        print('FUN3D FUNtoFEM finite-difference result (elastic): ', cv_product)
        #Thermal Terms
        cv_product_t = 0.0
        cv = body.aero_heat_flux.imag/epsilon
        cv_product_t += np.dot(cv, body.dQdfta[:, 0])
        print('FUN3D FUNtoFEM adjoint result (thermal):           ', adjoint_product_t)
        print('FUN3D FUNtoFEM finite-difference result (thermal): ', cv_product_t)
        return 
